...
class Main2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnInserta?.setOnClickListener {
            val ventanaTarea = Intent(this,DeCompraInsertar::class.java)
    ...
    fun mostrar(){
        ...
        try {
            ...
            var con = "SELECT * FROM DETALLECOMPRA"
            var cur = trans.rawQuery(con,null)
            var encEnLiTa = "ID     Descrcipción"
           mostrarTodasLi?.setText(encEnLiTa)
            if(cur != null){
        ...
        }catch (er: java.sql.SQLException){
    ...
    fun buscar(id: String, btnEtiqueta: String){
        try {
            var trans = Basedatos.readableDatabase
            var con="SELECT * FROM DETALLECOMPRA WHERE IDDETALLECOMPRA="+id
            var  cur = trans.rawQuery(con,null)
